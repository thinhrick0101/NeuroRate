...
def collate_fn(batch, tokenizer=None, max_length=512, device="cuda"):
    ...
    batch_dict = {
        "texts": texts,
        "attention_mask": torch.ones((len(texts), max_length, max_length), device=device)
    }
...
def main():
    ...
    # Create datasets
    train_dataset = TextDataset(train_texts, train_labels, max_length=config.max_position_embeddings)
    val_dataset = TextDataset(val_texts, val_labels, max_length=config.max_position_embeddings)
    ...
    train_loader = DataLoader(
        ...
        batch_size=8, 
        shuffle=True,
        collate_fn=lambda batch: collate_fn(batch, max_length=config.max_position_embeddings, device=device)
    )
